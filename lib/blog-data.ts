...
export const blogPosts: BlogPost[] = [
  ...
  },
  {
    ...
    content: [
      ...
      "An example, so this stays concrete. Picture a museum that places a limited run of AR objects around specific sculptures, unlockable only for holders of its membership pass NFT. It would never be enormous in headline numbers, but it is exactly the kind of use that shows institutional buyers this format is not a gimmick. Anchored NFTs at cultural venues are one of the most obvious near-term applications.",
      "Another. Picture a brand placing claimable NFTs at its own events, presence verified via camera, minted straight into the wallet. The interesting part is the drop-off analysis that follows: every claim comes from someone who verifiably turned up, which no promotional email of a comparable size can say. People walk to things when they can hold them.",
      "Our own publisher deals through Seekprotocol have followed the same pattern at smaller scale. Retail brands place limited-edition assets at flagship stores; music festivals anchor collectibles at stages; tourism boards run self-guided AR routes with a reward at each stop. The volumes are still small in absolute terms and the category is early, but the pattern is consistent: somebody who has physically walked to collect an NFT is far more likely to open the app again the next week than somebody who received the same asset as a passive airdrop.",
     ...
     pattern into /seekar, /business and /whitepaper. */
  {
    ...
    content: [
      ...
      "Pokémon Go is still the giant. Ten years in, more monthly players than any three of the others combined, and a live-events calendar that fills a city block on Community Day weekends. It is also, still, a closed economy: every rare creature you catch stays on Niantic's servers and cannot leave. If the ceiling of that fact has not started to matter to you yet, you have not been playing long enough. When it does, you will look at the rest of this list.",
...

  {
    ...
    content: [
      ...
      "Layer two, continued: rate limits and per spawn caps. Two collect requests less than five seconds apart get a 429 from a Postgres backed gate that survives across serverless isolates. A single spawn accepts at most two attempts from the same account, with the second attempt worth 65 percent of the first. These caps are not there for fraud detection; they are there so that a bot that already beat the other layers cannot mine the endpoint faster than a person can play.",
...

  {
    ...
    content: [
      ...
      "An example with numbers, so the metric is concrete. Suppose a coffee chain places a five-euro credit at 34 branches over a Wednesday-to-Sunday launch week, with a total budget of around eighteen thousand euros including the platform fee. If 1,700 people verifiably pick it up, the cost per verified arrival is about ten and a half euros, and every one of those arrivals comes with a wallet the chain can reach again. That is the number to hold against in-store CPMs from any other channel, which buy attention without the connection at the end.",
  ...
  },
  {
    ...
    content: [
      ...
      "Using advanced geofencing technology, Seekprotocol enables precise placement of digital assets at specific GPS coordinates, claimable only by someone within about 40 metres of the spot. From bustling city centers to hidden gems off the beaten path, every location becomes a potential reward zone waiting to be discovered.",
      ...
      "The proof of location system checks every claim against where the phone really is before anything is paid out. Bots and farms are what it is built to catch, so the rewards go to real people discovering them in real locations. This creates a level of authenticity and engagement that traditional distribution methods simply cannot match.",
...
/** The words of a post, which is what a translation replaces. */
export type BlogCopy = Pick<BlogPost, "title" | "excerpt" | "content" | "imageAlt" | "readTime" | "category">;
