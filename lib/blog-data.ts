...
export const blogPosts: BlogPost[] = [
...

  {
    ...
    title: "How Proof-of-Location Works (and Why GPS Alone Is Not Enough)",
    excerpt:
      "Proof-of-location is the difference between a real-world reward and a spoofed one. A plain walkthrough of the four signals a serious verifier combines, what each catches on its own, and where the whole stack still falls short.",
    content: [
      "The reason most location-based crypto ideas from 2020 quietly died is that they trusted GPS. GPS is a broadcast signal any receiver can listen to; a receiver can also be told to lie about what it heard, and the tooling to do that has been thirty dollars on Amazon since 2015. Any protocol that pays a wallet for standing at a coordinate, and verifies the standing with GPS alone, will lose money to fake wallets standing on couches. It is not a hard attack. It is not even a clever attack.",
      "Proof-of-location is the name for the layer that fixes this. It is one of those phrases that gets used more often than it gets defined, so this piece defines it plainly: proof-of-location is a claim, verifiable by a party who was not present, that a specific device was at a specific coordinate within a specific time window. It is a claim about presence, not about the coordinate. That distinction is where most of the engineering lives.",
      "The threat model, first. What we are defending against is not a nation-state — this is the ceiling case we will get to later — but the median attacker with a phone, a laptop, and forty minutes. This attacker can spoof GPS with an off-the-shelf tool, forge a device fingerprint with an emulator, and mint a wallet in one line of code. Any single signal we defend with can be beaten by this attacker. The trick is that beating multiple signals at once, in a way they agree on, is not thirty dollars of work — it is either professional-attacker time or specialised hardware neither of which pencils out against a two-euro reward.",
      "Signal one: satellite fix. The GPS constellation, plus Galileo, plus GLONASS, plus BeiDou. Multi-constellation receivers have been standard on phones since around 2021; a serious verifier reads all four and treats them as four separate signals rather than one. Spoofing all four constellations simultaneously and consistently is harder than spoofing GPS alone, but it is not impossible — the equipment costs a few hundred dollars now. The satellite fix is the base layer and it is the least trustworthy of the four we will describe.",
      "Signal two: ambient radio environment. Every phone can see nearby WiFi network BSSIDs, cell towers, and Bluetooth devices even without connecting to any of them. This produces a fingerprint of the physical location — the specific SSIDs of the coffee shop next door, the tower ID of the cell you are hitting, the presence of a particular Bluetooth beacon in the venue. Reproducing this fingerprint remotely requires knowing what the target location currently looks like, which is public in aggregate but expensive to keep current for every location a protocol supports. A spoofer who has never been to your park cannot fake the SSIDs your park sees.",
      "Signal three: device attestation. Apple's App Attest and Google's Play Integrity return a signed statement that says: this request came from an unmodified device, running an untampered version of the app, over a real cellular or WiFi connection. A rooted phone fails this. A phone in an emulator fails this. A phone with the app's binary modified to skip the location check fails this. Attestation does not prove where the device is — it proves the device is not lying about being a device — and that is the piece that makes the other signals mean something.",
      "Signal four: motion trace. Accelerometer and gyroscope history from the minutes before a claim. A phone that walked to a coordinate has a motion signature different from a phone that teleported to it. Continuous small movements, a natural step cadence, brief stops, direction changes — all of it hard to fake in a way that survives correlation with the GPS trace and the ambient radio changes. This is the signal that catches the attacker who spoofed everything else but did not simulate the walk that got them there.",
      "Each signal, on its own, can be beaten. That is the honest starting point. Signal one is beaten by a GPS simulator. Signal two by a spoofed radio environment (rare, expensive). Signal three by a jailbroken device with tampered libraries. Signal four by a scripted motion trace played back into the sensor stack. What is not cheaply beaten is all four at once, agreeing with each other, on a device that also passes attestation. The engineering premise is that the cost of forging the intersection exceeds the reward at any drop worth placing.",
      "The rule Seekprotocol's verifier uses is four-of-four with tolerance windows on each. A claim requires all four signals to agree within their respective bounds — satellite fix within thirty metres of the drop, radio fingerprint matching a recent scan of the area, attestation valid, motion trace showing arrival by ordinary means. A signal that disagrees rejects the claim rather than downweighting it. The whitepaper describes the tolerance math in detail; the point here is that the rule is strict rather than probabilistic.",
      "A worked example. A tester tried to claim a drop from an office chair in Berlin, aimed at a coordinate in Amsterdam. The GPS spoofer put the phone at the correct coordinate. The radio environment was Berlin's — five WiFi networks the drop had never seen. The claim rejected on signal two. Second attempt, with a script generating fake WiFi BSSIDs at the target location. The BSSIDs did not match Seekprotocol's crawl of the actual area from the week before. Still rejected. Third attempt, using a physical setup that broadcast the correct BSSIDs — the tester had built a small radio replica of the drop location on their desk. The motion trace was flat: the phone had never moved during the walk it claimed to have taken. Rejected on signal four. Fourth attempt was abandoned because the tester lost interest, which is the actual point of the design.",
      "The economic frame. What matters is not that spoofing is impossible — no defence in any category is — but that spoofing is expensive relative to the reward. A drop worth two euros protected by a stack that costs four hundred euros to forge is safe against the attacker who exists. A drop worth ten thousand euros protected by the same stack is not, and that drop needs additional verification steps. The design principle is match the trust budget to the value at risk, and be honest about the ceiling.",
      "What still does not work well. Indoor GPS is bad enough that a drop inside a shopping mall requires a Bluetooth beacon or a QR fallback to make the coordinate meaningful at all — no amount of clever ambient-radio verification fixes the fact that the satellite fix is off by fifteen metres. Wearables are still small in installed base; a proof-of-location stack designed for a phone with all its sensors does not transfer cleanly to a pair of glasses with a subset. And sub-second latency is not there yet — a verifier that reads four signals and cross-references them cannot return in under three hundred milliseconds today.",
      "The ceiling case, briefly. A nation-state with a truck full of RF equipment and a team of engineers can defeat proof-of-location as it exists today; there is nothing on the horizon that changes that in less than half a decade. This is the correct place for the ceiling: any protocol that claims to defeat state-level attackers with a mobile-phone verifier is lying. Ours does not.",
      "The design principle worth stating out loud. A verifier should be honest about what it can and cannot prove. Presenting a probabilistic score as an absolute yes/no is worse than presenting an absolute yes/no derived from strict rules, because the score creates a slider that publishers will push in the direction of accepting more claims when they want more engagement. The strict version has no slider and is boring in a good way.",
      "This is the piece under Seekprotocol that made anchored NFTs and location-verified airdrops work at all. It is why a publisher will put a token worth actual money at a coordinate — because the person who ends up with it was verifiably there. The [whitepaper](/whitepaper) has the full specification, including the tolerance math and the attack matrix. This piece is the plain-language version of what is in there.",
      "The short version. Presence is not a coordinate. A single signal does not prove presence. Four signals, agreeing with each other, on an attested device, do. The forgery cost of that intersection is what makes a reward worth placing. That is what proof-of-location actually is, and it is why the layer matters more than any of the flashier things it enables.",
    ],
    image: "/app/seekar-icon.svg",
    imageWidth: 1024,
    imageHeight: 1024,
