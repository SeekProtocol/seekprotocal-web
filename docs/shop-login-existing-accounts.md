...
Updated 2026-09-15. The shop uses the same Supabase project and user IDs as the app: `ukoqxyoogjouhdqhtiei`. It has no registration flow and does not link identities based only on a matching email address.
...
- Eligibility follows the app's onboarding rule: an active user with a profile and completed birthdate setup, or a legacy profile created before 2026-08-12. Anonymous, deleted, banned and suspended accounts are refused. No onboarding/registration forms are exposed by the shop.
...
- Existing Google/Apple/Discord credentials copied internally from the project's Auth configuration to `SHOP_AUTH_<PROVIDER>_CLIENT_ID` and `_CLIENT_SECRET`; values are never exposed in browser code or this document.
...
**Social providers remain disabled until their callback registrations are completed.** `SHOP_AUTH_READY_PROVIDERS` is currently empty. The website changes are local and have not been committed or published by this task.
...
Google's unauthenticated setup probe returned `redirect_uri_mismatch`; Apple's returned `invalid_request`. Discord could not be verified without its developer console. Supabase's own redirect allowlist does not configure the Google/Apple/Discord developer registrations.

For Apple, verify that the configured client is the Web Services ID associated/grouped with the native app, and that its client-secret JWT matches that Services ID and has not expired. This preserves the provider subject used by existing app accounts.
